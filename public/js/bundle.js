...
if (mapBox) {
    ...
    } else console.error("Mapbox script not loaded from CDN");
}
...
if (bookBtn) bookBtn.addEventListener("click", (e)=>{
    ...
    if (tourId && stripePublicKey) // Call bookTour
    (0, _stripe.bookTour)(tourId, stripePublicKey);
    else {
        target.textContent = "Book tour now!";
        (0, _alerts.showAlert)("error", "Missing critcal checkout parameter data.");
    }
});
...
if (forgotForm) forgotForm.addEventListener("submit", async (e)=>{
    ...
    if (emailEl) {
        ...
        await (0, _retrievePassword.forgotPassword)(email);
        // Clear input field
        forgotForm.reset();
    }
});
...
if (resetForm) resetForm.addEventListener("submit", async (e)=>{
    ...
    const passwordEl = document.getElementById("password");
    const passwordConfirmEl = document.getElementById("password");
...
if (reviewForm) reviewForm.addEventListener("submit", async (e)=>{
    ...
    if (tourId && ratingEl && textEl) {
        await (0, _review.createReview)(tourId, textEl.value, +ratingEl.value);
        // Clear input fields
        reviewForm.reset();
    }
});
...
},{"./mapbox":"70jYB","./login":"dFhXt","./signup":"jcO56","./retrievePassword":"4cM7I","./review":"eGj5R","./updateSettings":"eLvlg","./stripe":"fXgmC","./alerts":"5QDBO"}],"70jYB":[function(require,module,exports,__globalThis) {
...
const logout = async ()=>{
    try {
        ...
        console.log(err);
        // Safe Axios checking
        if ((0, _axios.isAxiosError)(err) && err.response) console.log(err.response);
...
const forgotPassword = async (email)=>{
    try {
        ...
        if (res.data.status === "success") (0, _alerts.showAlert)("success", "Token sent to email! Check your inbox.");
        // Redirect back home after a brief delay
        window.setTimeout(()=>{
            location.assign("/");
        }, 2000);
    } catch (err) {
        if ((0, _axios.isAxiosError)(err) && err.response?.data) (0, _alerts.showAlert)("error", err.response.data.message || "Failed to dispatch reset ling.");
        else (0, _alerts.showAlert)("error", "Network connection failed. Please try again.");
    }
};
const resetPassword = async (token, data)=>{
    try {
        ...
        if (res.data.status === "success") {
            (0, _alerts.showAlert)("success", "Password updated successfully! Redirecting to accout...");
...
const createReview = async (tourId, review, rating)=>{
    try {
        ...
        if (res.data.status === "success") {
            (0, _alerts.showAlert)("success", "Thank you! Your review has been saved.");
            window.setTimeout(()=>{
                location.reload();
            }, 1500);
        }
    } catch (err) {
        if ((0, _axios.isAxiosError)(err) && err.response?.data) (0, _alerts.showAlert)("error", "Network error. Review submission failed.");
    }
};
...
var _axiosDefault = parcelHelpers.interopDefault(_axios);
var _alerts = require("./alerts");
const bookTour = async (tourId, stripePublicKey)=>{
    try {
        //  Acess the global Stripe function via window casting
        const StripeFunc = window.Stripe;
        // Guard clause
        if (!StripeFunc) throw new Error("Stripe script failed to load from CDN!");
        ...
        if ((0, _axios.isAxiosError)(err) && err.response?.data) (0, _alerts.showAlert)("error", err.response.data.message || "Checkout transaction failed.");
        else if (err instanceof Error) (0, _alerts.showAlert)("error", err.message);
        else (0, _alerts.showAlert)("error", "An unknown error occured during trasactional preparation.");
    }
};
